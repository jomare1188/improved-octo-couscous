...
df3 <- data.frame(matrix(ncol = 3, nrow = 10))
colnames(df3) <- c("Contrast", "Type", "DEGs") 
# contrastast 5_M_D_vs_8_M_D
# up
df3[1,1] <- "M_D"
df3[1,2] <- "Up" 
df3[1,3] <- dim(contrast_1[contrast_1$log2FoldChange > 1,])[1]
names <- rownames(contrast_1[contrast_1$log2FoldChange > 1,])
write.table(names, paste0(contrast_results_dir, "/","5_M_D_vs_8_M_D", "/", "5_M_D_vs_8_M_D_up"), col.names = F, row.names = F, quote = F)
# down
df3[2,1] <- "M_D"
df3[2,2] <- "Down" 
df3[2,3] <- dim(contrast_1[contrast_1$log2FoldChange < 1,])[1]
names <- rownames(contrast_1[contrast_1$log2FoldChange < 1,])
write.table(names, paste0(contrast_results_dir, "/","5_M_D_vs_8_M_D", "/","5_M_D_vs_8_M_D_down"), col.names = F, row.names = F, quote = F)
# contrastast 5_T_D_vs_8_T_D
# up
df3[3,1] <- "T_D"
df3[3,2] <- "Up"
df3[3,3] <- dim(contrast_2[contrast_2$log2FoldChange > 1,])[1]
names <- rownames(contrast_2[contrast_2$log2FoldChange > 1,])
write.table(names, paste0(contrast_results_dir, "/","5_T_D_vs_8_T_D", "/", "5_T_D_vs_8_T_D_up"), col.names = F, row.names = F, quote = F)
# down 
df3[4,1] <- "T_D"
df3[4,2] <- "Down"
df3[4,3] <- dim(contrast_2[contrast_2$log2FoldChange < 1,])[1]
names <- rownames(contrast_2[contrast_2$log2FoldChange < 1,])
write.table(names, paste0(contrast_results_dir, "/","5_T_D_vs_8_T_D", "/","5_T_D_vs_8_T_D_down"), col.names = F, row.names = F, quote = F)
...
df3$Contrast <- factor(df3$Contrast, levels = c("8M_W-D", "M_W", "5M_W-D", "M_D", "T_D"))
...
ggsave(DEG_PLOT, filename = "../results/DEG_stackplot.svg", device= "svg" ,units = "cm",width = 52/1.5, height = 17,dpi = 320)
