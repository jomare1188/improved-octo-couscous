...
contrast_results_dir <- "/Storage/data1/jorge.munoz/DOLORES/RNAseq/results/contrasts"
log2foldthreshold <- 2
#contrast 1
...
#contrast_4 <-as.data.frame(read.table(paste0(contrast_results_dir,"/","5_M_W_vs_5_T_D", "/", "5_M_W_vs_5_T_D.DESeq.DE_results"), header = T) %>% filter(abs(log2FoldChange) > log2foldthreshold ) %>% filter((padj) < 0.01),header = F)

#contrast_5 <-as.data.frame(read.table(paste0(contrast_results_dir,"/","8_M_W_vs_8_T_D", "/", "8_M_W_vs_8_T_D.DESeq.DE_results"), header = T) %>% filter(abs(log2FoldChange) > log2foldthreshold ) %>% filter((padj) < 0.01),header = F)
...
ggsave(DEG_PLOT, filename = "../results/TDvsTW_DEG_stackplot.png", device= "png" ,units = "cm",width = 45/1.5, height = 17,dpi = 320)
